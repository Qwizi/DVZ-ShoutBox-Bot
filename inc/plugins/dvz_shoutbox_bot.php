...
$plugins->add_hook('member_do_register_end', ['dvz_shoutbox_bot', 'dvz_shoutbox_bot_action_register']);
$plugins->add_hook('datahandler_post_insert_thread_end', ['dvz_shoutbox_bot', 'dvz_shoutbox_bot_action_thread']);
$plugins->add_hook('datahandler_post_insert_post_end', ['dvz_shoutbox_bot', 'dvz_shoutbox_bot_action_post']);
...
function dvz_shoutbox_bot_install()
{
   ...
   $settings  = [
      ...
      [
         ...
         'title'             => 'Ignorowanie for',
         'description'  => 'Wybierz z listy fora, którę mają być ignorowane przez bota',
...
class dvz_shoutbox_bot
{
   ...
   function dvz_shoutbox_bot_action_post()
   {
      ...
      {
         $sql = 'SELECT pid, tid, fid, username, subject, dateline FROM '.TABLE_PREFIX.'posts ORDER BY dateline DESC LIMIT 1';
         $result = $db->query($sql);
         $row = $db->fetch_array($result);
